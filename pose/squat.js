/* =====================================================================================
   ...
   required return to the entry state, users farm reps by vibrating at the bottom.
     ...
     1. valid entry   STANDING (knee > standKnee)
     ...
     4. must return   knee back above standKnee, or no rep
...
export function create(){
  return {
    ...
    deepest:180, sawBottom:false, badTorso:false, frozen:false,
    reset(){ this.state='IDLE'; this.stateT=0; this.repT=0; this.phase=0;
             this.deepest=180; this.sawBottom=false; this.badTorso=false; },
  };
}
...
export function step(m, body, cal, ev){
  ...
  if (body.visMin.squat < CONFIG.common.visGate){
    m.frozen = true;
  ...
  switch (m.state){
    case 'IDLE':
      if (knee > C.standKnee){ m.state='STANDING'; m.stateT=0; }
      break;

    case 'STANDING':
      m.phase = 0;
      if (knee < C.standKnee - HYST){
        m.state='DESCENDING'; m.stateT=0; m.repT=0;
        m.deepest=knee; m.sawBottom=false; m.badTorso=false;
      }
      break;

    case 'DESCENDING':
      m.deepest = Math.min(m.deepest, knee);
      m.phase = span01(knee, C.standKnee, bottomKnee)*0.5;
      ev.progress = true; ev.phase = m.phase;
      if (body.torsoTilt > C.torsoMaxTilt) m.badTorso = true;
      if (knee < bottomKnee){ m.state='BOTTOM'; m.stateT=0; }
      else if (knee > C.standKnee - HYST*0.5){
    ...
    case 'ASCENDING':
      m.phase = 0.5 + span01(knee, bottomKnee, C.standKnee)*0.5;
      ev.progress = true; ev.phase = m.phase;
      if (knee > C.standKnee){
